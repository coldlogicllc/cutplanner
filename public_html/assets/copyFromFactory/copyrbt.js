...
RBT.nester_poly_x = [];
RBT.nester_poly_y = [];
RBT.nester_run = function (marker) {
    if (marker) {
        RBT.nester_marker = marker;
        RBT.nester_stop_requested = false;
    } else if (!RBT.nester_marker) {
        return;
    }
    //var e = document.getElementById("ds_div");
    if (RBT.nester_stop_requested) {
        RBT.status("RBT.nester_run stopped by RBT.nester_stop_requested");
    } else {
        RBT.nester_async_script('?get=ds&millis=' + RBT.nester_move_millis, null);
    }
};

RBT.nester_poly = function (id, g, x, y, overlaps) {
    var e = document.getElementById("svg_nester");
    e.innerHTML += g;
    RBT.nester_poly_x[id] = 0;
    RBT.nester_poly_y[id] = 0;
    RBT.nester_move(id, x, y, overlaps);
};
RBT.nester_polygons = function (args) {
    var e = document.getElementById("svg_nester");
    for (var a = 0; a < args.length; a++) {
        var id = args[a][0];
        RBT.nester_poly_x[id] = 0;
        RBT.nester_poly_y[id] = 0;
        e.innerHTML += args[a][1];
        var y_extent = args[a][2];
        var eid = document.getElementById(id);
        eid.setAttributeNS(null, "transform", "translate(0," + (-y_extent) + ")");
    }
    RBT.nester_run();
};
RBT.nester_move_millis = 500;
RBT.nester_move = function (id, x, y, overlaps) {
    var e = document.getElementById(id);
    if (!e) {
        alert("Not found " + id);
        setTimeout("RBT.nester_run();", RBT.nester_move_millis);
        return;
    }
    e.style.fill = overlaps ? "red" : "yellow";

    var ida = id + "animation";
    var animate = "<animateTransform xlink:href='#" + id + "' id='" + ida + "'attributename='transform' attributetype='XML' begin='indefinite' type='translate' "
            + " from='" + RBT.nester_poly_x[id] + "," + RBT.nester_poly_y[id] + "'"
            + " to='" + x + "," + y + "' dur='" + (RBT.nester_move_millis / 1000) + "s' repeatCount='1' fill='freeze' />";
    RBT.nester_poly_x[id] = x;
    RBT.nester_poly_y[id] = y;
    RBT.remove_by_id(ida);
    e.innerHTML += animate;
    // RBT.beginElement_by_id(ida);
    var eida = document.getElementById(ida);
    eida.setAttributeNS(null, "onend", "RBT.nester_run();");
    eida.beginElement();
    //setTimeout("RBT.nester_run();",millis);
};
RBT.nester_move_list = function (args) {
    var begin = 'indefinite';
    var starter_id = null;
    for (var a = 0; a < args.length; a++) {
        var id = args[a][0];
        var x = args[a][1];
        ...
        var ida = id + "animation";
        var animate = "<animateTransform xlink:href='#" + id + "' id='" + ida + "'attributename='transform' attributetype='XML' begin='" + begin + "' type='translate' "
                + " from='" + RBT.nester_poly_x[id] + "," + RBT.nester_poly_y[id] + "'"
                + " to='" + x + "," + y + "' dur='" + (RBT.nester_move_millis / 1000) + "s' repeatCount='1' fill='freeze' />";
        RBT.nester_poly_x[id] = x;
        RBT.nester_poly_y[id] = y;
    ...
    var eida = document.getElementById(starter_id);
    eida.setAttributeNS(null, "onend", "RBT.nester_run();");
    eida.beginElement();
...
RBT.nester_merge = function (g) {
    //RBT.remove_by_id("svg_merge");
    var e = document.getElementById("svg_nester");
    e.innerHTML += g;
    RBT.nester_run();
};
RBT.nester_overlap = function (x, y, cell_size) {
    var id = "svg_overlap";
    var idrect = id + "_rect";
    var e = document.getElementById(id);
    if (!e) {
        var outside = document.getElementById("svg_nester");
        outside.innerHTML += "<g id='" + id + "'><rect id='" + idrect + "' style='fill:blue;stroke:none;stroke-width:0px' x=0 y=0 height='" + cell_size + "' width='" + cell_size + "' /> </g>";
        e = document.getElementById(id);
    }
    e.setAttributeNS(null, "transform", "translate(" + x + ',' + y + ")");

//    var ida = id + "animation";
//    var animate = "<animateTransform xlink:href='#" + idrect + "' id='" + ida + "' attributename='transform' attributetype='XML' begin='indefinite' type='scale' "
//            + " from='2,2'"
//            + " to='1,1' dur='" + (RBT.nester_move_millis / 1000) + "s' repeatCount='indefinite' fill='freeze' />";
//    RBT.remove_by_id(ida);
//    e.innerHTML += animate;
//    var ea = document.getElementById(ida);
//    //ea.setAttributeNS(null, "onend", "RBT.nester_run();");
//    ea.beginElement();
    RBT.nester_run();
};
RBT.nester_check = function (f) {
    var form = f.form;
    var p1 = form.p1.value;
    var p2 = form.p2.value;
    RBT.nester_async_script('?check=overlap&p1=' + p1 + "&p2=" + p2, null);
};
RBT.nester_listen_level = function (f) {
    var p1 = f.options[f.selectedIndex].value;
    RBT.nester_async_script('?listen_level=' + p1, null);
};
RBT.nester_stop = function (f) {
    RBT.nester_marker = f.form.marker.value;
    RBT.nester_stop_requested = true;
    RBT.nester_async_script('?stop=stop', null);
};
RBT.nester_set = function (f) {
    RBT.nester_move_millis = parseInt(f.value, 10);
};
RBT.nester_merge_show = function (f) {
    var e = document.getElementById("svg_nester");
    if (e) {
        var style = f.checked ? "fill:pink;stroke-width:0;opacity:.1" : "fill:none;stroke-width:0;opacity:0";
        var gs = e.getElementsByClassName("svg_merge");
        for (var i = 0; i < gs.length; i++) {
            gs[i].setAttributeNS(null, "style", style);
        }

    } else {
        RBT.status("no merge available");
    }
};
RBT.nester_async_script = function (url) {
    RBT.async_script(url + '?marker=' + RBT.nester_marker, null);
};
